...
for(s in c('A','B','C')){
  ...
  for(i in 1:length(Gammas)){
    ...
    saveRDS(DTRES,file=paste0('D:\\files\\R new\\Precison_Medicine\\ALLRES_rdata\\DT_R_',scenario_used,'_',gamma_used,'.RData'))
    
    ###N
    DTRES<-DTfit(odat,vdat,method='N')
    saveRDS(DTRES,file=paste0('D:\\files\\R new\\Precison_Medicine\\ALLRES_rdata\\DT_N_',scenario_used,'_',gamma_used,'.RData'))
...
###visualization
ggdata<-c()   #sub for certain scenario 'A' 'B' 'C'
ggATE<-c()
...
for(i in 1:length(Gammas)){
  ...
  DTRES<-readRDS(file=paste0('D:\\files\\R new\\Precison_Medicine\\ALLRES_rdata\\DT_R_',scenario_used,'_',gamma_used,'.RData'))
  ggdata<-rbind(ggdata,
                    c(  mean( ( DTRES$v_predict - vdat$true_STE   )^2   ),gamma_used ,scenario_used, 'R', 'DT'  )
  )
  DTRES<-readRDS(file=paste0('D:\\files\\R new\\Precison_Medicine\\ALLRES_rdata\\DT_N_',scenario_used,'_',gamma_used,'.RData'))
  ggdata<-rbind(ggdata,
  ...
  ALLRES<-readRDS(file=paste0('D:\\files\\R new\\Precison_Medicine\\ALLRES_rdata\\RFQT_R_',scenario_used,'_',gamma_used,'.RData'))
  ggdata<-rbind(ggdata,
                    c(  ALLRES$MSE_test[length(ALLRES$MSE_test)] ,gamma_used ,scenario_used, 'R', 'RFQT'  )
  )
  ALLRES<-readRDS(file=paste0('D:\\files\\R new\\Precison_Medicine\\ALLRES_rdata\\RFQT_N_',scenario_used,'_',gamma_used,'.RData'))
  ggdata<-rbind(ggdata,
...
p<- ggplot(data = ggdata, mapping = aes(x = Gammas, y = MSE))+
     ...
     geom_line(  data=ggATE ,  mapping = aes(x = Gammas, y = MSE), linetype=3  )+
     geom_point(  data=ggATE ,  mapping = aes(x = Gammas, y = MSE) )+
     ...
     guides(  color=guide_legend(title='Stratification Method'),
       ...
       group=guide_legend(title='Stratification Method')  , linetype=guide_legend(title='Estimation Method')  ) #+coord_cartesian(ylim = c(0,6) )  
p
